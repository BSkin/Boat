...
int Game::initObjects()
{
	...
	oceanHeightModObjects = new list<GameObject*>();
	RenderObject::setStatics(&view, &projection, assetManager);
...
{
	delete oceanHeightModBuffer;
	oceanHeightModObjects->clear();
...
int Game::initBullet()
{
	...
	dynamicsWorld->setGravity(btVector3(0 , -9.81*2.0f ,0));
...
int Game::loadLevel(string input)
{
	...
	//new MeshObject("Models\\testramp.obj", "Textures\\rock_redDeepCuts_2k_d.tga", "Shaders\\default.glsl",
	...
	player = new PlayerObject("Models\\player.obj", "Textures\\error.tga", "Shaders\\default.glsl", 
		glm::vec3(-100,50,0), glm::vec3(0, 0, -1), glm::vec3(0, 1, 0), "sphere2");
	...
	*/
	...
	boat = new Boat("Models\\boat.obj", "Textures\\error.tga", "Shaders\\default.glsl", 
		glm::vec3(-100,20,0), glm::vec3(0, 0, 1), glm::vec3(0, 1, 0), "boatcollision");

	new Boat("Models\\boat.obj", "Textures\\error.tga", "Shaders\\default.glsl", 
		glm::vec3(100,20,0), glm::vec3(1, 0, 0), glm::vec3(0, 1, 0), "boatcollision");
	
	testbox = new GameObject("Models\\error.obj", "Textures\\error.tga", "Shaders\\default.glsl", 
		glm::vec3(0,0,0), glm::vec3(0, 0, -1), glm::vec3(0, 1, 0));
	testbox2 = new GameObject("Models\\error.obj", "Textures\\error.tga", "Shaders\\default.glsl", 
		glm::vec3(0,0,0), glm::vec3(0, 0, -1), glm::vec3(0, 1, 0));
	testbox3 = new GameObject("Models\\error.obj", "Textures\\error.tga", "Shaders\\default.glsl", 
		glm::vec3(0,0,0), glm::vec3(0, 0, -1), glm::vec3(0, 1, 0));
	testbox4 = new GameObject("Models\\error.obj", "Textures\\error.tga", "Shaders\\default.glsl", 
		glm::vec3(0,0,0), glm::vec3(0, 0, -1), glm::vec3(0, 1, 0));
	testbox5 = new GameObject("Models\\error.obj", "Textures\\error.tga", "Shaders\\default.glsl", 
		glm::vec3(0,0,0), glm::vec3(0, 0, -1), glm::vec3(0, 1, 0));
...
int Game::update(long elapsedTime)
{
	...
	if (isKeyDown(IM_UP)) {
		glm::vec3 boatVel = boat->getDirection()*10000.0f;
		boat->applyImpulseForward(1000.0f, elapsedTime);
	}
	if (isKeyDown(IM_LEFT)) {
		boat->turn(10000.0f, elapsedTime);
	}
	if (isKeyDown(IM_RIGHT)) {
		boat->turn(-10000.0f, elapsedTime);
	}

	if (isKeyDown(IM_NUM4))	ocean->modAmplitude(0.01f*elapsedTime);
	if (isKeyDown(IM_NUM1))	ocean->modAmplitude(-0.01f*elapsedTime);

	if (isKeyDown(IM_NUM5))	ocean->modWindSpeed(0.1f*elapsedTime);
	if (isKeyDown(IM_NUM2))	ocean->modWindSpeed(-0.1f*elapsedTime);

	if (isKeyDown(IM_NUM6))	ocean->modLambda(0.1f*elapsedTime);
	if (isKeyDown(IM_NUM3))	ocean->modLambda(-0.1f*elapsedTime);

	if (isKeyDown(IM_NUM8))	ocean->modGridSize(0.1f*elapsedTime);
	if (isKeyDown(IM_NUM9))	ocean->modGridSize(-0.1f*elapsedTime);
	
	dynamicsWorld->stepSimulation((float)elapsedTime*0.001f, 50, (float)elapsedTime*0.001f); //1.0f/(float)Settings::getMaxFPS());
	
	testbox->setPosition(100, ocean->getHeight(100, 100), 100);
	testbox->updateMatrices();
	testbox2->setPosition(40, ocean->getHeight(40, 40), 40);
	testbox2->updateMatrices();
	testbox3->setPosition(40, ocean->getHeight(40, 50), 50);
	testbox3->updateMatrices();
	testbox4->setPosition(30, ocean->getHeight(30, 30), 30);
	testbox4->updateMatrices();
	testbox5->setPosition(50, ocean->getHeight(50, 40), 40);
	testbox5->updateMatrices();

	for (list<GameObject*>::iterator it = gameObjects->begin(); it != gameObjects->end(); it++)
		(*it)->update(elapsedTime);

	updateBoatHeights();
	ocean->updateFinalOceanHeights(oceanHeightModBuffer->getDepthTexture(), heightModGridSize);
